...
function loadDataFromSample(request){
    console.log('Running loadDataFromSample')
...
function displayDataFromMemory(){
  if (log) {console.log('//////////////////// displayDataFromMemory ////////////////////')}
...
function buildCode(){
    ...
    if (finalPath && finalPath.length > 0) {
        ...
        finalPath.forEach((pathLine,i) => {
            if (i < finalPath.length - 1 ) { sampleCode += pathLine + '/' }
            else { sampleCode += pathLine }
        })
        sampleCode += '"\n'
    }
    // -- method
    ...
    if (hasData.query(finalQuery)) {
            ...
            finalQuery.forEach((queryLine,i) => {
                if (i < finalQuery.length - 1 ) {
                    // fix single quote
                    if (queryLine.indexOf('\'') >= 0) { queryLine=queryLine.replaceAll('\'','\\\'') }
                    // TODO: Do I need to fix double quotes and others?
                    sampleCode += queryLine + '&' 
                } 
                else { sampleCode += queryLine }
            })
            sampleCode += '"\n'
    }
        ...
        finalHeader.forEach((headerLine,i) => {
            ...
            if (headerLine.key.includes('Content-Type')) {
              ...
              if (showSystemHeaders) {
                ...
                sampleCode += '//   ' + prefix + '.AddHeader("' + headerLine.key + '","' + headerLine.value + '")\n'
              }
    ...
    if (finalAuth) {
        ...
        // console.log(finalAuth)
          ...
          sampleCode += prefix + '.AddHeader("Authorization","Bearer {' + prefix + '_AUTHTOKEN' + '}")\n'
        }
...
function buildHeader(){
    ...
    let hrLine = '//////////////////////////////////////////////////////////////////////' + '\n'
    let title = '//▂▃▅▇█▓▒░ Mark\'s Nice Sudio Snippet CodeGen 【ツ】 v' + version + ' ░▒▓█▇▅▃▂'  + '\n'
    let brLine = '\n'
    sampleCode += copyStart + hrLine + title + hrLine + brLine
    if (insidePostman) {sampleCode += "// " + pm.info.requestName + '\n\n'}
}
